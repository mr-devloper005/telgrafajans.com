...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[var(--editable-radius-card)] bg-[var(--editable-border-strong)]', className)} />
}

export function PageLoadingState({ label = 'Loading the guide', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-[var(--editable-container-wide)] px-4 py-16 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[var(--editable-radius-card)] border border-[var(--editable-border-strong)] p-5">
  ...
  return (
    <div className={cn('grid gap-6 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[var(--editable-radius-card)] border border-[var(--editable-border-strong)] p-5">
          <PulseBlock className="h-48 w-full" />
          <PulseBlock className="mt-5 h-5 w-5/6" />
          <PulseBlock className="mt-3 h-4 w-2/3" />
          <PulseBlock className="mt-6 h-10 w-32 rounded-[var(--editable-radius-button)]" />
        </div>
      ))}
    </div>
  )
...
export function DetailLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto grid w-full max-w-6xl gap-10 px-4 py-16 lg:grid-cols-[0.8fr_1.2fr]', className)} aria-live="polite" aria-busy="true">
